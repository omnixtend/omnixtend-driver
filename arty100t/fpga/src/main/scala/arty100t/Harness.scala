...
// Ethernet
////////////////////////////////////////////////////////////////////////////////
  val ethOverlay = dp(ArtyEthernetOverlayKey).head.place(ArtyEthernetDesignInput(
	dp(ExtTLMem).get.master.base, 
	dutWrangler.node, 
	harnessSysPLLNode
	)).asInstanceOf[EthernetArtyPlacedOverlay]
  
  val ethClient = TLClientNode(Seq(TLMasterPortParameters.v1(Seq(TLMasterParameters.v1(
    name = "ethernet",
    sourceId = IdRange(64, 65)
  )))))
  val ethBlockDuringReset = LazyModule(new TLBlockDuringReset(4))
  ethOverlay.overlayOutput.eth := ethBlockDuringReset.node := ethClient
...
// OmniXtend
////////////////////////////////////////////////////////////////////////////////
  val oxClient = TLClientNode(Seq(TLMasterPortParameters.v1(Seq(TLMasterParameters.v1(
    name = "arty_ox_core",
    sourceId = IdRange(80, 100)
  )))))
  ethOverlay.overlayOutput.oxnode := oxClient
////////////////////////////////////////////////////////////////////////////////
  ...
  class HarnessLikeImpl extends Impl with HasHarnessInstantiators {
    ...
    ethOverlay.ethernet.module.clock	:= harnessBinderClock
    ethOverlay.ethernet.module.reset	:= harnessBinderReset
    ethBlockDuringReset.module.clock := harnessBinderClock
    ethBlockDuringReset.module.reset := harnessBinderReset.asBool 
