...
class WithArtyEthernetTL extends HarnessBinder({
  case (th: HasHarnessInstantiators, port: TLEthPort, chipId: Int) => {
    val artyTh = th.asInstanceOf[LazyRawModuleImp].wrapper.asInstanceOf[Arty100THarness]
    val bundles = artyTh.ethClient.out.map(_._1)
    ...
    ethClientBundle <> port.io
  }
})

class WithArtyOmniXtendTL extends HarnessBinder({
  case (th: HasHarnessInstantiators, port: TLOXPort, chipId: Int) => {
    val artyTh = th.asInstanceOf[LazyRawModuleImp].wrapper.asInstanceOf[Arty100THarness]
    val bundles = artyTh.oxClient.out.map(_._1)
