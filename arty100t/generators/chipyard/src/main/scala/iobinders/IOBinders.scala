...
import chipyard.{CanHaveMasterTLMemPort, ChipyardSystem, ChipyardSystemModule, CanHaveMasterTLOXPort, CanHaveMasterTLEthPort}
...
class WithTLOXPunchthrough extends OverrideIOBinder({
  (system: CanHaveMasterTLOXPort) => {
    val io_tl_mem_pins_temp = IO(DataMirror.internal.chiselTypeClone[HeterogeneousBag[TLBundle]](system.ox_tl)).suggestName("ox_tl_slave")
    io_tl_mem_pins_temp <> system.ox_tl
    (Seq(TLOXPort(() => io_tl_mem_pins_temp)), Nil)
  }
})

class WithTLEthPunchthrough extends OverrideIOBinder({
  (system: CanHaveMasterTLEthPort) => {
    val io_tl_mem_pins_temp = IO(DataMirror.internal.chiselTypeClone[HeterogeneousBag[TLBundle]](system.eth_tl)).suggestName("eth_tl_slave")
    io_tl_mem_pins_temp <> system.eth_tl
    (Seq(TLEthPort(() => io_tl_mem_pins_temp)), Nil)
  }
})
